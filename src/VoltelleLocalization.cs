...
namespace MVolt.Rebuild
{
    ...
    internal static class VoltelleLocalization
    {
        private static readonly Dictionary<string, string> English = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            { "42 个 QueryInterface 入口与实机验证状态", "42 QueryInterface entries and hardware validation status" },
            ...
            { "PCI、总线与架构信息", "PCI, bus and architecture information" },
            { "PCI 位置", "PCI location" },
            ...
            { "SYS Clock 偏移", "SYS Clock offset" },
            { "Video Clock 偏移", "Video Clock offset" },
            { "风扇控制", "Fan control" },
            { "V/F 曲线", "V/F curve" },
            ...
            { "PerfClientLimits domain 6 使用 1,500,000 µV 控制哨兵；该数值不是工作电压。", "PerfClientLimits domain 6 uses a 1,500,000 µV control sentinel; this value is not an operating voltage." },
            ...
            { "显卡风扇控制", "GPU fan control" },
            { "每个 cooler 独立切换自动/手动；手动 duty 写入后立即回读", "Switch each cooler independently between Auto and Manual; manual duty is read back immediately after writing" },
            { "驱动未返回 ClientFanCoolers 控制通道。", "The driver did not return any ClientFanCoolers control channel." },
            { "ClientFanCoolers · 仅保存手动通道", "ClientFanCoolers · manual channels only" },
            { "手动控制", "Manual control" },
            { "目标 duty", "Target duty" },
            ...
            { "GPU 名称匹配 Blackwell REL-only 兼容规则；保存的配置档会记录 mobile_rel_only=true。", "The GPU name matches the Blackwell REL-only compatibility rule; saved profiles record mobile_rel_only=true." },
            ...
            { "配置文件使用稳定的 v1 schema，并绑定当前 GPU 名称和 VBIOS。加载待应用不会执行硬件写入。", "Profiles use a stable v1 schema and are bound to the current GPU name and VBIOS. Loading a target does not write hardware." },
            { "已保存配置", "Saved profiles" },
            { "启动行为：仅 GET 当前驱动状态", "Startup behavior: GET current driver state only" },
            { "启动行为：计划任务延迟自动应用指定配置档", "Startup behavior: scheduled task applies the selected profile after a delay" },
            { "配置档名称", "Profile name" },
            { "新建", "New" },
            { "保存当前", "Save current" },
            ...
            { "延迟开机自启", "Delayed startup" },
            { "登录后自动应用当前所选配置档并收进托盘", "Apply the selected profile after sign-in and minimize to the tray" },
            { "登录后延迟", "Delay after sign-in" },
            { "秒", "seconds" },
            { "保存自启设置", "Save startup settings" },
            ...
            { "配置文件", "Profile file" },
            { "原子替换并保留上一版 .bak；普通启动只读取，只有计划任务实例自动应用", "Atomic replacement with the previous .bak retained; normal launches are read-only at startup and only the scheduled-task instance auto-applies" },
            { "普通启动只读取", "Normal startup is read-only" },
            { "手动启动始终先执行 NVAPI GET 且不会自动写入；仅当用户启用上方计划任务时，登录后的专用实例会在设定延迟后应用指定配置档。", "A manual launch always performs NVAPI GET first and never writes automatically. Only the dedicated post-sign-in instance applies the selected profile after the configured delay when the scheduled task is enabled." },
            { "接口入口集合已实现", "Interface entry set implemented" },
            { "当前驱动可用性", "Current driver availability" },
            { "可用", "Available" },
            { "实现与验证状态", "Implementation and validation" },
            { "入口 ID", "Entry IDs" },
            { "10 个直接 ID + 32 个私有 ID", "10 direct IDs + 32 private IDs" },
            { "安全与布局测试", "Safety and layout tests" },
            { "SET 接口族", "SET interface families" },
            { "实机写入项目", "Hardware write checks" },
            { "核心功能与新增 SYS、Video、Fan 已完成最小写入和 GET 回读", "Core features plus the new SYS, Video and Fan controls passed minimum writes and GET readback" },
            { "普通托盘实例只维持实时遥测；仅用户启用的登录计划任务会延迟自动应用指定配置档。", "Normal tray instances maintain telemetry only. Only a user-enabled sign-in task applies the selected profile after a delay." },
            ...
            { "缓冲区、草稿保留、失败继续、报告、配置档与异常路径", "Buffers, draft preservation, continue-after-failure behavior, reports, profiles and exception paths" },
            ...
            { "当前实现使用 42 个唯一 QueryInterface ID；其中 0x527FC458 用于 ClockDomains MeasureFrequency。", "The current implementation uses 42 unique QueryInterface IDs; 0x527FC458 is used for ClockDomains MeasureFrequency." },
            ...
            { "报告已写入：\n", "Report written to:\n" },
            { "这是 NV Voltelle 生成的明文 GET-only 兼容报告。\n\n", "This is a plaintext GET-only compatibility report generated by NV Voltelle.\n\n" },
            ...
            { "UI 验收完成：单项应用刷新已执行，其他草稿保持不变；测试构建未发送任何 SET。", "UI acceptance passed: the single-item refresh ran and other drafts remained unchanged; the test build sent no SET calls." },
        ...
        private static readonly KeyValuePair<string, string>[] EnglishFragments = new[]
        {
            ...
            Pair("此选项不可用", "This option is unavailable"),
            Pair("接口、状态或范围不可用", "interface, status, or range is unavailable"),
            Pair("接口、点表或范围不可用", "interface, point table, or range is unavailable"),
            Pair("接口或状态不可用", "interface or status is unavailable"),
            Pair("接口不可用", "interface is unavailable"),
            Pair("配置档 “", "Profile “"),
            Pair("制作者 ", "Created by "),
            Pair("B站 @", "Bilibili @"),
            Pair("驱动 ", "Driver "),
            ...
            Pair("保存开机自启设置失败：", "Failed to save startup settings: "),
            Pair("已启用开机自动应用：“", "Enabled startup auto-apply: “"),
            Pair("已关闭开机自动应用并删除计划任务。", "Disabled startup auto-apply and removed the scheduled task."),
            Pair("开机配置档已部分应用：成功 ", "Startup profile partially applied. Successful: "),
            Pair("开机配置档已应用并回读：", "Startup profile applied and read back: "),
            Pair("开机自动应用未执行：", "Startup auto-apply was not run: "),
            Pair("开机自动应用失败：", "Startup auto-apply failed: "),
            Pair("风扇 duty 必须是 ", "Fan duty must be "),
            Pair("风扇 duty 超出驱动实时范围。", "Fan duty is outside the live driver range."),
